...
impl EnvBase for Guest {
    ...
    fn escalate_error_to_panic(&self, s: Self::Error) -> ! {
        unimplemented!()
    }
...
impl EnvBase for Guest {
    ...
    fn escalate_status_to_error(&self, s: Status) -> Self::Error {
        let _ = self.fail_with_status(s);
        // We never get to this line, but we need to manifest a type `!`
        // somehow, and the 1-byte `0x00` unreachable wasm insn is the simplest
        // non-empty option. Other options would be `loop {}` which is 1 insn
        // longer and `unsafe { core::hint::unreachable_unchecked() }` which is
        // 1 byte smaller but has the disadvantage of being UB if we're wrong
        // and control ever accidentally _does_ get here.
