use stellar_contract_env_common::TryIntoVal;
...
fn tuple_roundtrip() -> Result<(), HostError> {
    let host = Host::default();
    let t0: (u32, i32) = (5, -4);
    let ev = t0.into_val(&host);
    let t0_back: (u32, i32) = ev.try_into_val(&host)?;
