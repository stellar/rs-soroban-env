...
fn u8_array() -> Result<(), HostError> {
    ...
    assert_eq!(1u32, TryInto::<u32>::try_into(host.bytes_get(obj, 0u32.into())?)?);
    assert_eq!(2u32, TryInto::<u32>::try_into(host.bytes_get(obj, 1u32.into())?)?);
    assert_eq!(3u32, TryInto::<u32>::try_into(host.bytes_get(obj, 2u32.into())?)?);
