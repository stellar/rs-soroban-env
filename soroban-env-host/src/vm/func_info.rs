...
pub(crate) struct HostFuncInfo {
    ...
    /// Same but for wasmi_036::Linker
    pub(crate) wrap_036: fn(
        &mut wasmi_036::Linker<Host>,
    )
        -> Result<&mut wasmi_036::Linker<Host>, wasmi_036::errors::LinkerError>,
...
macro_rules! host_function_info_helper {
    {$mod_str:literal, $fn_id:literal, $args:tt, $func_id:ident, $min_proto:literal, $max_proto:literal} => {
        HostFuncInfo {
            ...
            wrap_036: |linker| linker.func_wrap($mod_str, $fn_id, dispatch::dispatch_036::$func_id),
