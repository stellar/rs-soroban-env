    ...
    W: Write,
{
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        self.host
            .charge_budget(CostType::ValSer, 1, Some(buf.len() as u64))
...
impl Host {
    ...
    pub(crate) fn metered_hash_xdr(&self, obj: &impl WriteXdr) -> Result<[u8; 32], HostError> {
        ...
        self.charge_budget(CostType::ComputeSha256Hash, 1, Some(buf.len() as u64))?;
        Ok(Sha256::digest(buf).try_into()?)
    }

    pub(crate) fn metered_from_xdr<T: ReadXdr>(&self, bytes: &[u8]) -> Result<T, HostError> {
        self.charge_budget(CostType::ValDeser, 1, Some(bytes.len() as u64))?;
