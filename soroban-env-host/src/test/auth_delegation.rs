...
use crate::DEFAULT_HOST_DEPTH_LIMIT;
use crate::{
...
impl AuthBuilder {
    ...
    fn build_auth_entries(
        ...
        let network_id = self
            ...
            .try_into()
            .unwrap();
        let use_address_v2_creds = self.use_preimage_with_address.is_none()
            && !self.has_builtin_delegated_signers
            && !self.force_credential_with_delegates
            && thread_rng().gen::<bool>();
        let use_preimage_with_address = if let Some(v) = self.use_preimage_with_address {
            v
        } else {
            self.has_builtin_delegated_signers || use_address_v2_creds
        };
        ...
        let curr_auth_entry = if delegates.is_empty() && !self.force_credential_with_delegates {
            let credentials = if use_address_v2_creds {
...
fn test_builtin_delegated_signers_must_be_present_in_payload() {
    ...
    // `AddressWithDelegates` credentials but the `delegates` list is empty,
    // and the contract still asks to delegate.
    assert!(HostError::result_matches_err(
        test.call_auth_fn(
            test.new_auth_builder(1)
                .force_credential_with_delegates()
                .add_builtin_delegated_sig(&test.contracts[2])
        ),
        (ScErrorType::Auth, ScErrorCode::InvalidAction)
    ));
}
...
fn test_delegated_signers_getter() {
    ...
    // No delegated signers (plain `Address` credentials — no tracker).
    assert!(test
        .call_auth_fn(test.new_auth_builder(1).expect_delegated_signers(&[]))
        .is_ok());
    // `AddressWithDelegates` credentials with an empty `delegates` list — the
    // tracker exists but has no children, so the getter must still return an
    // empty vec.
    assert!(test
        .call_auth_fn(
            test.new_auth_builder(1)
    ...
    // A consumed delegate must still be returned by the getter. The signature
    // order is preserved so `delegate_account_auth(2)` runs before the getter.
    assert!(test
        .call_auth_fn(
            test.new_auth_builder(1)
                .preserve_signature_order()
                .sign_builtin_delegated(test.new_auth_builder(2).expect_delegated_signers(&[]))
                .expect_delegated_signers(&[&test.contracts[2], &test.contracts[4]])
                .sign_builtin_delegated(test.new_auth_builder(4).expect_delegated_signers(&[]))
        )
        .is_ok());
...
#[test]
fn test_failed_nested_auth_does_not_change_current_auth_frame() {
    let test = DelegatedAuthTest::setup(5);
    ...
    let res = test.call_auth_fn(
        test.new_auth_builder(1)
            ...
            .expect_delegated_signers(&[&test.contracts[4]])
            .sign_builtin_delegated(test.new_auth_builder(4).expect_delegated_signers(&[])),
    );
...
#[test]
fn test_deeply_nested_delegate_tree_is_rejected() {
    let run_test = |depth: u32| {
        let test = DelegatedAuthTest::setup(1);
        test.host.budget_ref().reset_unlimited().unwrap();
        let address_sc = test.contracts[0].to_sc_address().unwrap();

        let mut current = SorobanDelegateSignature {
            address: address_sc.clone(),
            signature: ScVal::Void,
            nested_delegates: vec![].try_into().unwrap(),
        };
        // Make the delegate signature to have depth of `depth - 1`.
        for _ in 0..depth - 2 {
            current = SorobanDelegateSignature {
                address: address_sc.clone(),
                signature: ScVal::Void,
                nested_delegates: vec![current].try_into().unwrap(),
            };
        }

        let root_invocation = SorobanAuthorizedInvocation {
            function: SorobanAuthorizedFunction::ContractFn(InvokeContractArgs {
                contract_address: address_sc.clone(),
                function_name: "auth_fn".try_into().unwrap(),
                args: vec![ScVal::Address(address_sc.clone()), AUTH_FN_ARG.into()]
                    .try_into()
                    .unwrap(),
            }),
            sub_invocations: Default::default(),
        ...
        let auth_entry = SorobanAuthorizationEntry {
            credentials: SorobanCredentials::AddressWithDelegates(
                SorobanAddressCredentialsWithDelegates {
                    ...
                    // Add 1 more to the depth of the overall entry (`current`
                    // is already at `depth - 1`).
                    delegates: vec![current].try_into().unwrap(),
                },
            ),
            root_invocation,
        };
        test.host.set_authorization_entries(vec![auth_entry])
    };
    // 1 less than depth limit should succeed, hitting the limit should fail.
    assert!(run_test(DEFAULT_HOST_DEPTH_LIMIT - 1).is_ok());
    assert!(HostError::result_matches_err(
        run_test(DEFAULT_HOST_DEPTH_LIMIT),
