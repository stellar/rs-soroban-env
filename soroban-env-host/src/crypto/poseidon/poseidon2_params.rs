...
impl<F: MeteredScalar> Poseidon2Params<F> {
    ...
    pub fn new(
        ...
        if mat_internal_diag_m_1.len() != t as usize {
            return Err(host.error(INVALID_INPUT, "mat_internal_diag_m_1 length does not match `t`", &[Val::from_u32(mat_internal_diag_m_1.len() as u32).into(), Val::from_u32(t).into()]));
        }
        if rounds_f % 2 != 0 {
            return Err(host.error(INVALID_INPUT, "`rounds_f` must be even", &[Val::from_u32(rounds_f).into()]));
        }
        let r = rounds_f / 2;
    ...
    pub fn new_unchecked(
        ...
        assert_eq!(mat_internal_diag_m_1.len(), t as usize, "mat_internal_diag_m_1 length {} does not match t={}", mat_internal_diag_m_1.len(), t);
