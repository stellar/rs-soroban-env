...
use soroban_env_common::{CheckedEnv, Convert, RawVal};
...
pub fn read_nonce(e: &Host, id: Identifier) -> Result<i128, HostError> {
    ...
    if let Ok(nonce) = e.get_contract_data(e.convert(key)?) {
        Ok(e.convert(nonce)?)
    } else {
        Ok(0)
    }
