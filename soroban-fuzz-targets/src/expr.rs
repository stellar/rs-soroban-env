...
// The idea of this is to fuzz use the soroban-synth-wasm expression-tree
// generator to generate wasms that exercise various host functions -- passing
// outputs of one host function to inputs of another, where argument and return
// types and function names and arities are driven from the host function
// definitions in env.json.
//
// In order to give it a somewhat broader set of inputs to explore, we generate
// two separate contracts A and B, and we give the fuzzer some control over the
// arguments passed to A (including giving it some byte string literals to play
// with, and the address of B, and some signer addresses) and then we let it
// generate arbitrary wasm bodies for A and B that use those arguments. We then
// fuzz for panics and internal errors in the host as it executes these
// generated contracts.

/// Test case structure for the expr fuzz target.
    ...
    fn install_budget(&self, host: &Host) {
        host.with_budget(|budget| {
            ...
            budget.reset_limits(
                ...
                self.mem_budget as u64 & 0x0fff_ffff | 0x000f_ffff,
            )
        })
        .unwrap();
    }
...
/// Run the expr fuzz target with the given input bytes.
///
/// This generates two contracts A and B from synthesized wasm expressions,
/// invokes contract A (which may call B), and checks for internal errors.
///
/// Returns `FuzzResult::Ok` if the test completed (even if the contract errored),
/// `FuzzResult::Reject` if the input was malformed/too short,
/// `FuzzResult::InternalError` if an internal error was detected (a bug).
pub fn run_fuzz_target(data: &[u8]) -> FuzzResult {
    let mut u = Unstructured::new(data);
    let test: ExprTestCase = match ExprTestCase::arbitrary(&mut u) {
        Ok(t) => t,
    ...
    if let Err(hosterror) = res {
        ...
        {
            return FuzzResult::InternalError;
        }
    }
