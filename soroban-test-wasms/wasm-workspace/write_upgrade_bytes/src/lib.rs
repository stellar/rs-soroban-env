...
pub(crate) const BALANCE_TTL_THRESHOLD: u32 = BUMP_AMOUNT;
...
impl WriteBytesContract {
    pub fn write(env: Env, xdr_bytes: Bytes) -> BytesN<32> {
        ...
        env.storage().temporary().extend(&hash, BALANCE_TTL_THRESHOLD, BUMP_AMOUNT);

        env.storage().instance().extend(BALANCE_TTL_THRESHOLD, BUMP_AMOUNT);
