...
impl Contract {
    ...
    pub fn bump_persistent(e: Env, key: Symbol, min_to_live: u32, threshold: u32) {
        e.storage().persistent().bump(&key, min_to_live, threshold)
    }
    ...
    pub fn bump_temporary(e: Env, key: Symbol, min_to_live: u32, threshold: u32) {
        e.storage().temporary().bump(&key, min_to_live, threshold)
    }
    ...
    pub fn bump_instance(e: Env, min_to_live: u32, threshold: u32) {
        e.storage().instance().bump(min_to_live, threshold)
    }
}
